...
# 두리안 추가 -> + 연산자 사용: 연결 연산자
fruits = fruits + ['두리안'] # 포도를 더해서 fruits에 저장
print("과일 목록(두리안추가): ", fruits)

num = [1, 2, 3] + [4, 5, 6] # 리스트에서 +는 연결이다.
print("숫자 리스트: ", num)

# *(곱하기) 연산자 -> 리스트를 n번 반복한다.
num = [1, 2, 3] * 3
print("숫자 리스트: ", num)
...
print("과일 리스트 중 1, 2, 3번 과일은?", fruits[0:3]) # 0번지~2번지
print("과일 리스트 중 1, 2, 3번 과일은?", fruits[:3]) # 처음부터 2번지
print("과일 리스트 중 3번부터 마지막 과일은?", fruits[2:])
print("과일 리스트 중 1, 3, 5번 과일은?", fruits[::2]) # 처음부터 끝까지 리스트 중에서 2씩 증가하면서..
...
#  원하는 위치에 샤인머스캣 추가 -> insert() 메소드
fruits.insert(3, '샤인머스캣')
print("과일 목록 (3번지에 추가): ", fruits)
